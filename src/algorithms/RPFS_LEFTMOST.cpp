// RPFS_LEFTMOST.cpp automatically generated by bin/add_new_algorithm.pl
// in Tue Oct 17 21:53:53 2017.

//
// RPFS_LEFTMOST.cpp -- implementation of the class "RPFS_LEFTMOST".
//
...
#include "RPFS_LEFTMOST.h"

RPFS_LEFTMOST::RPFS_LEFTMOST ()
{
...
void RPFS_LEFTMOST::get_minima_list (unsigned int max_size_of_minima_list)
{
  ...
  unsigned int * order;
  order = new unsigned int[set_card + 1];
  for (unsigned int i = 0; i < set_card; i++)
  {
    order[i] = set_card - i - 1;
    // cout << "order[" << i << "] = " << order[i] << endl;
  }
  order[set_card] = set_card;
...
PFSNode * RPFS_LEFTMOST::lower_forest_branch (ForestOBDD * Forest_A, 
  ForestOBDD * Forest_B)
{
  PFSNode * R, * M, * N;
  unsigned int i, m;
  ...
  // cout << "\n\nForest state: " << endl;
  // Forest_A->print ();
  // cout << "L Selected node " << R->vertex->print_subset () << endl;
...
PFSNode * RPFS_LEFTMOST::upper_forest_branch (ForestOBDD * Forest_A, 
  ForestOBDD * Forest_B)
{
  PFSNode * R, * M, * N;
  unsigned int i, m;
  ...
  // cout << "\n\nForest state: " << endl;
  // Forest_B->print ();
  // cout << "U Selected node " << R->vertex->print_subset () << endl;
...
void RPFS_LEFTMOST::search_upper_root (ForestOBDD * Forest_B, 
  ElementSubset * M)
{
  PFSNode * N;
  unsigned int i;
...
void RPFS_LEFTMOST::search_lower_root (ForestOBDD * Forest_A, 
  ElementSubset * M)
{
  PFSNode * N;
  unsigned int i;
...
void RPFS_LEFTMOST::search_lower_children (ForestOBDD * Forest_B, 
...
void RPFS_LEFTMOST::search_upper_children (ForestOBDD * Forest_A, 
  PFSNode * N, ElementSubset * M, ElementSubset * Y)
{
  int i;
  PFSNode * A;
...
void RPFS_LEFTMOST::upper_forest_pruning (ForestOBDD * Forest_B, 
  PFSNode * N)
{
  int k;
  PFSNode * _M;
...
void RPFS_LEFTMOST::lower_forest_pruning (ForestOBDD * Forest_A, 
  PFSNode * N)
{
  PFSNode * _M;
  ElementSubset M ("", set);
...
void RPFS_LEFTMOST::calculate_node_cost (PFSNode * R, ForestOBDD * Forest_Dual)
{
  if (R->cost == FLT_MAX)
  {
