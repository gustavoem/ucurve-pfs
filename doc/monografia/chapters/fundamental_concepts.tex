...
apresentam um formato em u nas cadeias do reticulado booleano. Vamos
definir tal propriedade agora.
...
Vamos considerar o caso particular do problema de seleção de 
características em que a função de custo utilizada é decomponível em 
curvas u. Este é o problema central deste trabalho.
...
Provamos que a função de soma de subconjuntos é decomponível em curvas 
u, mas o mesmo não necessariamente se aplica a função de custo MCE. 
Entretanto, é comum que o formato da curva dessa função sobre as cadeias
maximais do reticulado seja aproximadamente um u, ou seja, existem
poucos pontos que violam a definição \ref{fund_concepts:ushape}. Esse 
formato da curva pode ser explorado pelo algoritmo de busca do problema
de seleção de característica, diminuindo o número de nós visitados para
se descobrir o subconjunto de custo mínimo.
