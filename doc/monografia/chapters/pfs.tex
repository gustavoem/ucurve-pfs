...
\begin{algorithmic}[1]
\If{$\pfsnode{N}[\varname{cost}] = \infty$ and there is $\pfsnode{M} \in \forest$ such that $\pfsnode{M}[\varname{vertex}] = \pfsnode{N}[\varname{vertex}]$ and $\pfsnode{M}[\varname{cost}] \ne \infty$}
...
\begin{algorithm}[H]
    ...
    \begin{algorithmic}[1]
        \If{there is $\pfsnode{N} \in \forest$ such that $\pfsnode{N}[\varname{vertex}] = N$}
            \Return $\pfsnode{N}$
        \Else
            \Return $NIL$
        \EndIf
